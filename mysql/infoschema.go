...
// schema information and table data
...
// using 'SELECT *'. So spatial columns will be fetched using AsText().
...
func toType(dataType string, columnType string, charLen sql.NullInt64, numericPrecision, numericScale sql.NullInt64) schema.Type {
	switch {
	case dataType == "set":
		return schema.Type{Name: columnType, ArrayBounds: []int64{-1}}
	default:
		return schema.Type{Name: columnType}
	}
}
