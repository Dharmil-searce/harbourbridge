...
func processCreateStmt(conv *internal.Conv, n nodes.CreateStmt) {
	...
	for _, te := range n.TableElts.Items {
		...
		default:
			conv.Unexpected(fmt.Sprintf("Found %s node while processing CreateStmt TableElts", internal.PrNodeType(i)))
		}
	}
...
func processInsertStmt(conv *internal.Conv, n nodes.InsertStmt) *copyOrInsert {
	...
	default:
		conv.Unexpected(fmt.Sprintf("Found %s node while processing InsertStmt SelectStmt", internal.PrNodeType(sel)))
	}
	return nil
}
	...
	for _, x := range t.Items {
		...
		case nodes.A_Const:
			...
			case nodes.Integer:
				l = append(l, t2.Ival)
			default:
				conv.Unexpected(fmt.Sprintf("Found %s node while processing Typmods", internal.PrNodeType(t1.Val)))
			}
		default:
			conv.Unexpected(fmt.Sprintf("Found %s node while processing Typmods", internal.PrNodeType(x)))
		}
	}
	return l
}
...
func getArrayBounds(conv *internal.Conv, t nodes.List) (l []int64) {
	for _, x := range t.Items {
		...
		case nodes.Integer:
			...
			l = append(l, t.Ival)
		default:
			conv.Unexpected(fmt.Sprintf("Found %s node while processing ArrayBounds", internal.PrNodeType(x)))
		}
	}
	return l
}
