...
func ConvertData(conv *internal.Conv, srcTable string, srcCols []string, srcSchema schema.Table, spTable string, spCols []string, spSchema ddl.CreateTable, vals []string) (string, []string, []interface{}, error) {
	...
	for i, spCol := range spCols {
		...
		if spColDef.IsArray {
			...
			x, err = convScalar(spColDef.T, srcColDef.Type.Name, conv.Offset, vals[i])
		}
		if err != nil {
...
func convScalar(spannerType ddl.ScalarType, srcTypeName string, location string, val string) (interface{}, error) {
...
// pg_dump.
func convTimestamp(srcTypeName string, location string, val string) (t time.Time, err error) {
	...
	if srcTypeName == "timestamp" {
		if location == "" {
			location = "+00:00" // if timezone is not specified in mysqldump, we consider UTC time
		}
		...
		time_joined = time_joined + "" + location
...
func convArray(spannerType ddl.ScalarType, srcTypeName string, location *time.Location, v string) (interface{}, error) {
	...
	// Hence we have to do the following case analysis (Mysql only support array<string>)
	switch spannerType.(type) {
